...
class JetTagMVAExtractor : public edm::EDAnalyzer {
	...
	edm::EDGetTokenT<JetFlavourInfoMatchingCollection>					jetFlavour;
	edm::EDGetTokenT<edm::Association<reco::GenJetCollection> >					genJetsMatched;
	...
	std::map<std::string, edm::EDGetTokenT<edm::View<BaseTagInfo> > >		tagInfoLabels;
	std::vector<edm::EDGetTokenT<edm::View<BaseTagInfo> > >		tagInfos;
...
JetTagMVAExtractor::JetTagMVAExtractor(const edm::ParameterSet &params) :
	jetFlavour(consumes<JetFlavourInfoMatchingCollection>(params.getParameter<edm::InputTag>("jetFlavourMatching"))),
	genJetsMatched(consumes<edm::Association<reco::GenJetCollection> >(params.getParameter<edm::InputTag>("matchedGenJets"))),
	...
	mvaComputer(dummyCalib())
{
	...
	for(std::vector<std::string>::const_iterator iter = inputTags.begin(); iter != inputTags.end(); iter++)
		tagInfoLabels[*iter] = consumes<edm::View<BaseTagInfo> >(params.getParameter<edm::InputTag>(*iter));
		
	

	
	//added by Gerrit to make sure all trees are created beforehand; empty ones are better for running on the grid than no ones.
...
void JetTagMVAExtractor::setup(const JetTagComputer &computer)
{
	...
	for(std::vector<std::string>::const_iterator iter = inputLabels.begin();
		...
		std::map<std::string, edm::EDGetTokenT<edm::View<BaseTagInfo> > >::const_iterator pos =
...
void JetTagMVAExtractor::analyze(const edm::Event& event, const edm::EventSetup& es)
{
	...
	for(unsigned int i = 0; i < nTagInfos; i++) {
		...
		event.getByToken(tagInfos[i], tagInfoHandle);
	...
	event.getByToken(genJetsMatched, genJetMatch);
...
//	edm::Handle<JetFlavourMatchingCollection> jetFlavourHandle;
	...
	event.getByToken(jetFlavour, jetFlavourHandle);
